...
import 'package:pinch_to_zoom_scrollable/src/insistent_scale_gesture_recognizer.dart';
...
/// See also:
///
///   * [InsistentInteractiveViewer.builder], whose builder is of this type.
///   * [WidgetBuilder], which is similar, but takes no viewport.
typedef InsistentInteractiveViewerWidgetBuilder = Widget Function(
...
/// {@end-tool}
@immutable
class InsistentInteractiveViewer extends StatefulWidget {
  /// Create an InteractiveViewer.
  ///
  /// The [child] parameter must not be null.
  InsistentInteractiveViewer({
    Key? key,
  ...
  InsistentInteractiveViewer.builder({
    Key? key,
    this.clipBehavior = Clip.hardEdge,
    this.alignPanAxis = false,
    this.boundaryMargin = EdgeInsets.zero,
  ...
  ///
  /// Passed with the [InsistentInteractiveViewer.builder] constructor. Otherwise, the
  ...
  ///
  /// If the [AggressiveInteractiveViewer.builder] constructor is used, then this will be
  ...
  ///
  /// At the time this is called, the [AggressiveTransformationController] will have
  ...
  ///
  /// At the time this is called, the [AggressiveTransformationController] will not have
  ...
  /// [AggressiveTransformationController.toScene] for how to convert these coordinates to
  ...
  /// A [AggressiveTransformationController] for the transformation performed on the
  /// child.
  ///
  ...
  final InsistentTransformationController? transformationController;
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      InsistentInteractiveViewer.getNearestPointOnLine(
  ...
  @override
  State<InsistentInteractiveViewer> createState() =>
      _InsistentInteractiveViewerState();
}

class _InsistentInteractiveViewerState extends State<InsistentInteractiveViewer>
    with TickerProviderStateMixin {
  InsistentTransformationController? _transformationController;
  ...
  late final _factory =
      GestureRecognizerFactoryWithHandlers<InsistentScaleGestureRecognizer>(
    () => InsistentScaleGestureRecognizer()
  ...
  void initState() {
    ...
    _transformationController =
        widget.transformationController ?? InsistentTransformationController();
  ...
  @override
  void didUpdateWidget(InsistentInteractiveViewer oldWidget) {
    ...
    if (oldWidget.transformationController == null) {
      ...
      if (widget.transformationController == null) {
        _transformationController!
            .removeListener(_onTransformationControllerChange);
        _transformationController = InsistentTransformationController();
        _transformationController!
            .addListener(_onTransformationControllerChange);
  ...
  Widget build(BuildContext context) {
    ...
    Widget ui = RawGestureDetector(
      gestures: <Type, GestureRecognizerFactory>{
        InsistentScaleGestureRecognizer: _factory
      },
      child: child,
    );
...
/// See also:
///
///  * [InsistentInteractiveViewer.transformationController] for detailed documentation
///    on how to use TransformationController with [InteractiveViewer].
class InsistentTransformationController extends ValueNotifier<Matrix4> {
  /// Create an instance of [InsistentTransformationController].
  ///
  /// The [value] defaults to the identity matrix, which corresponds to no
  /// transformation.
  InsistentTransformationController([Matrix4? value])
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return InsistentInteractiveViewer.getAxisAlignedBoundingBox(
      boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside =
        InsistentInteractiveViewer.getNearestPointInside(point, boundary);
...
Offset _round(Offset offset) => Offset(
